...
public class PSPopupSingleSelectView extends LinearLayout {
    ...
    private ArrayList<PCategoryData> pCategoryDatas;
    ...
    */
    ...
    public PSPopupSingleSelectView(Context context, String title, ArrayList pData, String type) {
        super(context, null);
        Utils.psLog("3***");
        this.title = title;

        if(type.equals("city")) {
            ArrayList<PCityData> pCityDatas = pData;
            setItemsWithPCityData(pCityDatas);
        } else if(type.equals("category")) {
            ArrayList<PCategoryData> pCategoryDatas = pData;
            setItemsWithPCategoryData(pCategoryDatas);
        }
...
//                                .show();
                    ...
                    } else if(pCategoryDatas != null && pCategoryDatas.size() > 0) {
                        ...
                        new AlertDialog.Builder(getContext())
                                ...
                                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                                    public void onClick(DialogInterface dialog, int whichButton) {
                                        ...
                                        if (pCategoryDatas != null && pCategoryDatas.size() > 0) {
                                            onSelectListener.Select(null, selectedPosition, selectedText, pCategoryDatas.get(selectedPosition).id);
                                        }
                                    }
                                })
                       ...
                       /* MaterialDialog dialog = new MaterialDialog.Builder(getContext())
                                ...
                                .itemsCallbackSingleChoice(-1, new MaterialDialog.ListCallbackSingleChoice() {
                                    ...
                                    public boolean onSelection(MaterialDialog dialog, View view, int which, CharSequence text) {
                                        ...
                                        if (pCategoryDatas != null && pCategoryDatas.size() > 0) {
                                            onSelectListener.Select(view, which, text, pCategoryDatas.get(which).id);
                                        }
                                        return true;
                                    }
                                })
                        ...
                        Toast.makeText(getContext(), "There is no item to select.", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
    ...
    public void setItemsWithPCategoryData(ArrayList<PCategoryData> pCategoryDatas) {
        int i = 0;
        this.pCategoryDatas = pCategoryDatas;
        try {
            this.items = new CharSequence[pCategoryDatas.size()];
            Utils.psLog("setup *** >> " + items.length);

            if(pCategoryDatas != null && pCategoryDatas.size() > 0){
                for (PCategoryData attDetail : pCategoryDatas) {
                    this.items[i++] = attDetail.name.toString();
                    Utils.psLog("Cat Name *** >> " + attDetail.name.toString());
                }
            }
        } catch (Exception e) {
            Utils.psLog("Error");
